            ...
            this.each(function (i,ele) {
                if (!name) {
                    ...
                    if ($(ele).hasClass(name)) {
                        var clsA = ele.className.split(' ');
                        for (var i = 0; i < clsA.length; i++) {
                            clsA[i] === name && clsA.splice(i--, 1);
                        }
                        ele.className = clsA.join(' ');
                    };
            });
            return this;
        }
    ...
    ytSlider.prototype = {
        ...
        _slideLayouts: {
            ...
            left: function () {
                ...
                style.width = (sW * sC) + "px";
                style.left = "0px";

                if (sW< cW) {
                    var _sC = Math.ceil(sW * sC / cW);
                    me.scount = sW * sC % cW < sW ? _sC - 1 : _sC;
                    me.cnWidth = sW * (Math.round(cW / sW));
                }
            },
            ...
            top: function () {
                ...
                style.height = sH * sC + "px";
                style.top = "0px";
                if (sH < cH) {
                    var _sC = Math.ceil(sH * sC / cH);
                    me.scount = sH * sC % cH < sH ? _sC - 1 : _sC;
                    me.cnHeight = sH * (Math.round(cH / sH));
            ...
            fade: function () {
                $.each(this.scns, function (i,ele) {
                    ...
                    $(ele).css({ opacity: o, position: 'absolute', 'z-index': z, left: 0, top: 0,"display":i==0?"block":"none" });
                },this);
            }
        },
        ...
        _setDOM: function () {
            ...
            var me = this,
                cf = me.cf,
                box = cf.idBox.nodeType==1 ? cf.idBox : $("#" + cf.idBox)[0],
            ...
            if (me.sWidth < me.cnWidth) {
                me.cnWidth = scn.parentNode.offsetWidth;
            }
            if (me.sHeight < me.cnHeight) {
                me.cnHeight= scn.parentNode.offsetHeight;
            }
            //设置布局
